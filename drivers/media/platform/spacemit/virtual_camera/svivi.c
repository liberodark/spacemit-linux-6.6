...
#define SVIVI_MAX_WIDTH				1920
#define SVIVI_MAX_HEIGHT			1200
...
int send_to_user(struct sock *netlinkfd, void *data, unsigned int len, unsigned int pid, unsigned int seq)
{
	...
	vcam_dbg("send to user len:%d, seq:%d", nlh->nlmsg_len, nlh->nlmsg_seq);
...
int fill_recv_msg_by_nlmsg_data(struct nlmsghdr *nlh)
{
	...
	if (in_use == false) {
		vcam_warn("video had close, pid:%d, %d, don't respond netlink:%d",
	...
	if (data) {
		...
		case FINISH_INIT:
			memcpy(&recv_msg.rcv_vheader.vinit, data,
			       sizeof(struct v4l2_vinit));
			break;
		case FINISH_S_FMT:
			memcpy(&recv_msg.rcv_vheader.vfmt, data,
			       sizeof(struct v4l2_vformat));
			break;
		case FINISH_REQBUFS:
			memcpy(&recv_msg.rcv_vheader.vreq_buf, data,
			       sizeof(struct v4l2_vrequestbuffers));
			break;
		case FINISH_QUERYBUF:
			memcpy(&recv_msg.rcv_vheader.vbuf, data,
			       sizeof(struct v4l2_vbuffer));
			break;
		case FINISH_QBUF:
			memcpy(&recv_msg.rcv_vheader.vbuf, data,
			       sizeof(struct v4l2_vbuffer));
		...
		case FINISH_STREAMON:
			memcpy(&recv_msg.rcv_vheader.vstream, data,
			       sizeof(struct v4l2_vstream));
			break;
		case FINISH_POLL:
			memcpy(&recv_msg.rcv_vheader.vpoll, data,
			       sizeof(struct v4l2_vpoll));
			break;
		case FINISH_STREAMOFF:
			...
			break;
		case FINISH_CLOSE:
			memcpy(&recv_msg.rcv_vheader.vdeinit, data,
			       sizeof(struct v4l2_vdeinit));
			break;
		default:
			vcam_warn("user seq %d - 1 != kernel seq %d",
				  nlh->nlmsg_seq, snd_msg.snd_vheader.kpos);
		...
		vcam_dbg("recv from user pid %d, upos %d",
			 recv_msg.rcv_vheader.user_pid,
...
static int wait_for_recv_complete(int seq, int timeout)
{
	...
	if (!wait_for_completion_timeout(&recv_msg.complete, msecs_to_jiffies(timeout))) {
		vcam_err("timeout %d!! recv user ack failed, seq:%d", timeout, seq);
		return -EAGAIN;
	}
	if (recv_msg.rcv_vheader.upos != seq) {
		vcam_err("recv user ack state error, seq %d != %d",
			 recv_msg.rcv_vheader.upos, seq);
		return -EINVAL;
	}
	...
	case FINISH_STREAMOFF:
		...
		break;
	case FINISH_CLOSE:
		ret = recv_msg.rcv_vheader.vdeinit.ret;
		break;
	default:
		vcam_warn("unexpected resv happen!");
		return -1;
	...
	if (ret)
		vcam_err("user ack but run failed, ret:%d, seq:%d", ret,
			 recv_msg.rcv_vheader.upos);
...
const struct svivi_fmt_info *svivi_format_try(struct v4l2_pix_format_mplane *pix)
{
	...
	pix->width = clamp(pix->width, SVIVI_MIN_WIDTH, SVIVI_MAX_WIDTH);
	pix->height = clamp(pix->height, SVIVI_MIN_HEIGHT, SVIVI_MAX_HEIGHT);
...
static int svivi_s_fmt_vid_cap(struct file *file, void *priv, struct v4l2_format *f)
{
	...
	if (in_use == false) {
		vcam_warn("video had closed, plase open first");
		return 0;
	}
	...
	ret = wait_for_recv_complete(FINISH_INIT, 3000);
	if (ret)
		return ret;
	...
	send_to_user(vind->netlinkfd, &snd_vheader->vfmt,
		     sizeof(struct v4l2_vformat), snd_vheader->user_pid,
		     snd_vheader->kpos);

	ret = wait_for_recv_complete(FINISH_S_FMT, 3000);
	if (ret)
		return ret;
...
static int svivi_reqbufs(struct file *file, void *priv, struct v4l2_requestbuffers *p)
{
	...
	if (p->count >= 3) {
		...
		ret = wait_for_recv_complete(FINISH_REQBUFS, 3000);
		if (ret)
			return ret;
	} else {
		vcam_warn("reqbufs buf type: %d, memory: %d, count: %d", p->type, p->memory, p->count);
	}
...
static int svivi_dqbuf(struct file *file, void *priv, struct v4l2_buffer *p)
{
	...
	snd_vheader->user_pid = recv_msg.rcv_vheader.user_pid;
	send_to_user(vind->netlinkfd, &snd_vheader->vbuf,
		     sizeof(struct v4l2_vbuffer), snd_vheader->user_pid,
		     snd_vheader->kpos);
...
static int svivi_qbuf(struct file *file, void *priv, struct v4l2_buffer *p)
{
	...
	vcam_dbg("queue buf%d type:%d, memory:%d, fd:%d", p->index, p->type,
		 p->memory, p->m.planes[0].m.fd);
	mutex_lock(&vind->lock);
...
static int svivi_querybuf(struct file *file, void *priv, struct v4l2_buffer *p)
{
	...
	vcam_info("querybuff exit, index:%d %d fd:%d", p->index,
		  recv_msg.rcv_vheader.vbuf.index,
		  recv_msg.rcv_vheader.vbuf.m_fd);
...
static int svivi_streamon(struct file *file, void *priv, enum v4l2_buf_type i)
{
	...
	send_to_user(vind->netlinkfd, &snd_vheader->vstream,
		     sizeof(struct v4l2_vstream), snd_vheader->user_pid,
		     snd_vheader->kpos);

	ret = wait_for_recv_complete(FINISH_STREAMON, 6000);
	if (ret)
		return ret;
...
static int svivi_streamoff(struct file *file, void *priv, enum v4l2_buf_type i)
{
	...
	ret = wait_for_recv_complete(FINISH_STREAMOFF, 5000);
	if (ret)
		vcam_err("stream off return: %d", ret);
...
static int svivi_release(struct file *file)
{
	...
	vcam_info("close vnode(%s).", video_device_node_name(vnode));
	if (recv_msg.rcv_vheader.user_pid != 0) {
		snd_vheader->kpos = START_CLOSE;
		snd_vheader->user_pid = recv_msg.rcv_vheader.user_pid;
		send_to_user(vind->netlinkfd, &snd_vheader->vdeinit,
			     sizeof(struct v4l2_vdeinit), snd_vheader->user_pid,
			     snd_vheader->kpos);
		ret = wait_for_recv_complete(FINISH_CLOSE, 2000);
	}
	in_use = false;
...
static __poll_t svivi_poll(struct file *file, poll_table *wait)
{
	...
	if (dqbuf_num >= vind->req_count) {
		vcam_info("poll, but no qbuf, return EPOLLERR (%d, %d)",
	...
	snd_vheader->user_pid = recv_msg.rcv_vheader.user_pid;
	send_to_user(vind->netlinkfd, &snd_vheader->vpoll,
		     sizeof(struct v4l2_vpoll), snd_vheader->user_pid,
		     snd_vheader->kpos);

	ret = wait_for_recv_complete(FINISH_POLL, 3000);
...
static int svivi_probe(struct platform_device *pdev)
{
	...
	svivi->netlinkfd =
	    (struct sock *)netlink_kernel_create(&init_net, SVIVI_NETLINK, &cfg);
...
static int svivi_remove(struct platform_device *pdev)
{
	...
	video_unregister_device(&vind->vid_cap_dev2);
