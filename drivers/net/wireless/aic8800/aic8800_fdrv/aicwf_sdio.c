...
static int aicwf_sdio_suspend(struct device *dev)
{
	...
	while (1) {
		spin_lock_bh(&sdiodev->rwnx_hw->cmd_mgr->lock);
		queue_sz = sdiodev->rwnx_hw->cmd_mgr->queue_sz;
		spin_unlock_bh(&sdiodev->rwnx_hw->cmd_mgr->lock);

		if ((queue_sz != 0) || aicwf_wakeup_lock_status(sdiodev->rwnx_hw)) {
			msleep(100);
			wait_cnt--;
			sdio_dbg("%s wait ws release, wait count:%d\n", __func__, wait_cnt);
			if (wait_cnt == 0) {
				sdio_dbg("%s wait ws release fail\n", __func__);
				break;
			}
		} else {
			break;
