	...
	enum phl_band_idx hw_band)
{
	...
	//phl_sta_i = bb->phl_sta_info[macid];
	/* pwr : S(8,1)*/
	ret_v |= halbb_write_mask_pwr_reg_cmn(bb, (enum phl_phy_idx)hw_band, reg_ofst, mask_ccath, cca_th);
	ret_v |= halbb_write_mask_pwr_reg_cmn(bb, (enum phl_phy_idx)hw_band, reg_ofst, mask_en, cca_th_en);
...
void halbb_set_pwr_macid_idx(struct bb_info *bb, u16 macid, s8 pwr, bool pwr_en, u8 idx)
{
	...
	/* pwr : S(8,1)*/
	ret_v |= halbb_write_mask_pwr_reg_cmn(bb, (enum phl_phy_idx)hw_band, reg_ofst, mask_pwr, pwr);
	ret_v |= halbb_write_mask_pwr_reg_cmn(bb, (enum phl_phy_idx)hw_band, reg_ofst, mask_en, pwr_en);
...

{
	...
	for (i = 0; i < cr_size; i+=2) {
		...
		halbb_set_pwr_reg_cmn(bb, (enum phl_phy_idx)band, tpu_array[i], tpu_array[i+1]);
	}
