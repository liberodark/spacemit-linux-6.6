...
/* r8168_leds.c: Realtek 8168 ethernet driver.
 *
...
#define lcdev_to_r8168_ldev(lcdev) container_of(lcdev, struct r8168_led_classdev, led)

static int rtl8168_led_hw_control_is_supported(struct led_classdev *led_cdev,
					       unsigned long flags)
{
	struct r8168_led_classdev *ldev = lcdev_to_r8168_ldev(led_cdev);
	struct rtl8168_private *tp = netdev_priv(ldev->ndev);
				      ...
				      unsigned long flags)
{
	...
	rtl8168_led_mod_ctrl(tp, 0x000f << shift, mode << shift);
	return 0;
}
...
static struct device *
	r8168_led_hw_control_get_device(struct led_classdev *led_cdev)
{
	struct r8168_led_classdev *ldev = lcdev_to_r8168_ldev(led_cdev);
...
static void rtl8168_setup_ldev(struct r8168_led_classdev *ldev,
			       struct net_device *ndev, int index)
{
	...
	r8168_get_led_name(tp, index, led_name, LED_MAX_NAME_SIZE);
	...
	led_cdev->hw_control_get_device = r8168_led_hw_control_get_device;
...
struct r8168_led_classdev *rtl8168_init_leds(struct net_device *ndev)
{
	...
	rtl8168_led_mod_ctrl(tp, 0x7000, 0x7000);
...
void r8168_remove_leds(struct r8168_led_classdev *leds)
{
	...
	for (struct r8168_led_classdev *l = leds; l->ndev; l++)
