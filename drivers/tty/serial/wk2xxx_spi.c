...
/*************The debug control **********************************/
#if defined(CONFIG_WK2XXX_SPI_DEBUG)
...
#if defined(CONFIG_WK2XXX_SPI_RS485)
#define WK_RS485_FUNCTION
#endif

#if defined(CONFIG_WK2XXX_SPI_GPIO_RST)
#define WK_RSTGPIO_FUNCTION
#endif

#if defined(CONFIG_WK2XXX_SPI_GPIO_CS)
#define WK_CSGPIO_FUNCTION
#endif

#if defined(CONFIG_WK2XXX_SPI_GPIO_PWR)
...
/*enable rs485*/
	...
	//wk2xxx_write_slave_reg(spi, one->port.iobase, WK2XXX_RS485_REG, 0X02); //default  high
	wk2xxx_write_slave_reg(spi, one->port.iobase, WK2XXX_RS485_REG, 0X03); //default low
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 1, 0)
	sched_setattr_nocheck(priv->kworker_task, &attr);
#else
...
static const struct spi_device_id wkmic_spi_ids[] = {
    { "wk2124_spi", 0 },
    { "spi-wk2xxx", 1 },
	{ "wk2xxx_spi", 2 },
	{ }
};
MODULE_DEVICE_TABLE(spi, wkmic_spi_ids);
...
static struct spi_driver wk2xxx_spi_driver = {
    ...
    .id_table = wkmic_spi_ids,
};
module_spi_driver(wk2xxx_spi_driver);
